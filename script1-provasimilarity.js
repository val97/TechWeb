...
//            e.preventDefault();
      ...
      function grafica(data){
          for (var i = 0; i < data.length; i++) {
             $("#main").append("<button data-dismiss='modal' onclick='caricavideo("+JSON.stringify(data[i].videoID)+JSON.stringify(data[i].artist)+")'>"+data[i].category+ data[i].artist+data[i].title+data[i].videoID+"</button>");
          }
        }

        function caricavideo(data,artist){
            newClock();
            currentVideo=data;
            lastartist=artist;
       ...
       function stampa(vid, dim,category,info){
        ...
        var html="";
          ...
          $('img').click(function(){
                ...
                caricavideo($(this).attr("value"),$(this).attr("value"));
          });
        }
        ...
        function caricaTab(category) {
            ...
            if(category=="ArtistSimilarity"){
              ...
              if(resSearch.items[0].id.videoId!="undefined"){
                artistlist(lastartist);
                  dim=0;
                  caricavideo(resArt.items[0].id.videoId,resArt.item[0].artist);
                  console.log(resSearch.items[0].id.videoId);
              }
              //visualizzo la lista dei video cercati
              else{
                  dim=resArt.items.length;
                  for (var i = 0; i < dim; i++) {
                      vid[i]=resArt.items[i];
                      console.log(resArt.items[i]);
                  }
}
              stampa(vid,dim,category,info);
            }
            ...
            if (category=="search") {
              ...
              if(resSearch.items[0].id.videoId==$('#query').val()){
                  dim=0;
                  caricavideo(resSearch.items[0].id.videoId,resSearch.items[0].artist);
              }
              //visualizzo la lista dei video cercati
              else{
      ...
      function artistlist(artist){
        ...
        request.execute(function(response){
          ...
          $('#ArtistSimilarity-button').click(caricaTab('ArtistSimilarity'));
           console.log(response);
