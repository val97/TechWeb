          ...
          if(category== "ArtistSimilarity"){
            ...
            $.ajax({
                            ...
                            $.get("http://localhost:8000/ArtistSimilarity?title="+titolo[0].trim(), function(data, status){
                              ...
                              html += "<div  class='card border-info mb-3' style='width: 16rem;display: inline-block;'>";
                              html+=" <img class='card-img-top'  src ='https://img.youtube.com/vi/"+resArtSim[i]+"/default.jpg' value='"+resArtSim[i]+"' alt='Card image cap'>";
                              html += "<div class='card-body'> <p class='card-text'><b>"+ title +"</b><br>"+data.results.bindings[i].album+"<br>"+data.results.bindings[i].date+"</p></div></div> ";
                            }
                              $(".tabcontent#"+category).html(html);
                            });
