        ...
        if(data.results.bindings[0].songName){
          ...
          for(var j=0;j<recommender_size;j++){
           ...
           // alert("pippo");
            ...
            for (var i = 0; i < dim; i++) {
                  vid[i]=resSearch.items[i];*/
                  stampa(vid[20],dim,category,info);
            });
          }
            //stampa(vid,dim,category,info);
...
if(category== "GenreSimilarity"){
      ...
      $.ajax({
        url: 'https://www.googleapis.com/youtube/v3/videos?key=' + 'AIzaSyCmxhjyAdTBxuEOG_etapCgLYwIBpSmdbQ' + '&id=' + currentVideo + '&part=snippet',
          ...
          $.get("http://localhost:8000/Genre?title="+genere,"&artist=" +artist, function(data, status){

            if(data.results.bindings[0].artName){
                artist=data.results.bindings[j].artName;
                $.get("http://localhost:8000/GenreArtist?title="+titolo[0].trim(), function(data, status){
            ...
            url: 'https://www.googleapis.com/youtube/v3/videos?key=' + 'AIzaSyCmxhjyAdTBxuEOG_etapCgLYwIBpSmdbQ' + '&id=' + currentVideo + '&part=snippet',
                        ...
                        complete: function(){
                          ...
                          $.get("http://localhost:8000/BandSimilarity?title="+titolo[0].trim(), function(data, status){
                            var band="";
                            if(data.results.bindings.bandName){
                              band=data.results.bindings[0].bandName;
                            $.get("http://localhost:8000/GenreMembri?band="+band, function(data, status){
                              var membri="";
                              if(data.results.bindings[0].member){
                                membri=data.results.bindings[0].member;
                              $.get("http://localhost:8000/Band?membriBand="+membriBand+"&band="+band, function(data, status){
                                var songs="";
                                if(data.results.bindings[0].songName){
                                  for(var j=0;j<recommender_size;j++){
